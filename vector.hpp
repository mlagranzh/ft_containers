...
/*   Updated: 2022/04/29 15:58:36 by celys            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include <iostream>
#include "iterators/vec_RAI.hpp"
#include "iterators/rev_vec_RAI.hpp"
#include "utils.hpp"
    ...
    class vector 
    {
	    ...
	    public:
			...
			vector(InputIterator first, InputIterator last, const allocator_type& alloc = A(), 
				...
				: _arr(0), _len(0), _cap(0), _alloc(alloc)
			{
				if (first > last)
					throw std::length_error("vector");
				_len = last - first;
...
*/
			...
			iterator erase( iterator position )
			{
				...
				for (size_type i = d; i < _len - 1; ++i){
					_alloc.destroy(_arr + i);
					_alloc.construct(_arr + i, *(_arr + i + 1));
				}
				_len--;
				_alloc.destroy(_arr + _len - 1);
