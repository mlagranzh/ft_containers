	...
	class rev_map_iterator
	{
		...
		public:
			...
			rev_map_iterator(const rev_map_iterator<Key, T, Compare, Node, false>& other) {
				_node = other.getNonConstNode();
				_lastElem = other.getNonConstLastElem();
				_comp = other.getCompare();
			}

			explicit rev_map_iterator(map_iterator<Key, T, Compare, Node, false> other) {
				--other;
				_node = other.getNonConstNode();
				_lastElem = other.getNonConstLastElem();
				_comp = other.getCompare();
			...
			iterator	base() const { return this->_node; }
