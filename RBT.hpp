...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/29 15:44:01 by celys             #+#    #+#             */
/*   Updated: 2022/04/29 15:44:04 by celys            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include "memory"
#include "iterators/rev_map_RAI.hpp"
#include "iterators/map_RAI.hpp"
#include "utils.hpp"
...
namespace ft
{
	...
	public:
		...
		tree(const Compare &comp, const allocator_type& a = allocator_type()): _val_alloc(a), _node_alloc(node_allocator()), _compare(comp), _root(0), _size(0) {
			init_nil_head();
			_root = _header;
		}

		tree() : _root(0), _val_alloc(allocator_type()), _node_alloc(node_allocator()), _compare(value_compare()), _size(0) {
			init_nil_head();
			_root = _header;
		}

		tree(const tree& src) :  _compare(src._compare), _root(NULL) {
			*this = src;
		}

		template<class InputIt>
		tree(typename ft::enable_if< !ft::is_integral<InputIt>::value, InputIt >::type first, InputIt last,
